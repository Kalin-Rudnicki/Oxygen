...
final case class NonEmptyList[+A](head: A, tail: List[A]) extends PartialFunction[Int, A] {
  ...
  def ++[A2 >: A](that: NonEmptyList[A2]): NonEmptyList[A2] = NonEmptyList.unsafeFromList(this.toList ++ that.toList)
  def ++[A2 >: A](that: IterableOnce[A2]): NonEmptyList[A2] = NonEmptyList.unsafeFromList(this.toList ++ that)
