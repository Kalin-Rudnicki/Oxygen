...
object JavaCommandService extends CommandService {
  ...
  override def executeSync(
    ...
    for {
      outBuilder <- ZIO.succeed { StringBuilder.emptyThreadUnsafe }
      errBuilder <- ZIO.succeed { StringBuilder.emptyThreadUnsafe }
      exitCode <- run(
        command = command,
        stdIn = stdIn,
        stdOut = OutputMode.Collect(outBuilder),
        stdErr = OutputMode.Collect(errBuilder),
      )
    } yield (
      stdOut = maybeTrim(outBuilder.build(), trim),
      stdErr = maybeTrim(errBuilder.build(), trim),
      exitCode = exitCode,
    )
  ...
  private def writeInput(
    ...
    stdIn match {
      ...
      case CommandInputSource.Const(value) =>
        ...
        ZIO
          .attemptBlockingInterrupt { os.write(value.getBytes(charset)); os.flush() }
  ...
  private def consumeOutput(
    ...
    mode match {
      ...
      case OutputMode.Collect(builder) =>
        ZStream
          .fromInputStream(stream)
          .via(ZPipeline.utf8Decode)
          .runForeach { str => ZIO.succeed { builder.append(str) } }
          .convertCausesFail { executionFailure(command, "read process output", _) }

      case OutputMode.PipeTo(target) =>
        ZIO
          .attemptBlockingInterrupt { stream.transferTo(target) }
          .ensuring { ZIO.attempt { stream.close() }.ignore }
          .unit
  ...
  private enum OutputMode {
    case Discard
    case Collect(builder: StringBuilder)
