...
extension [RefT](self: FiberRef[RefT]) {

  private[zio] def locallyWithAnnotation(f: RefT => RefT): ZIOAspectPoly = ZIOAspectPoly.fiberRefLocallyWith(self)(f)
  private[zio] def locallyWithUIOAnnotation(f: RefT => UIO[RefT]): ZIOAspectPoly = ZIOAspectPoly.fiberRefLocallyWithUIO(self)(f)
  private[zio] def locallyAnnotation(f: RefT): ZIOAspectPoly = ZIOAspectPoly.fiberRefLocally(self)(f)
  private[zio] def locallyUIOAnnotation(f: UIO[RefT]): ZIOAspectPoly = ZIOAspectPoly.fiberRefLocallyUIO(self)(f)

  private[zio] def locallyWithURIOAnnotation[R](f: RefT => URIO[R, RefT]): ZIOAspectAtLeastR[R] = ZIOAspectAtLeastR.fiberRefLocallyWithURIO(self)(f)
  private[zio] def locallyURIOAnnotation[R](f: URIO[R, RefT]): ZIOAspectAtLeastR[R] = ZIOAspectAtLeastR.fiberRefLocallyURIO(self)(f)
