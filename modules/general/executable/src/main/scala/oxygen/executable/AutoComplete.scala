...
object AutoComplete {
  ...
  private def generateScriptText(
    ...
    s"""#!/usr/bin/env bash
       |
       |# Auto-generated by oxygen-executable. Requires bash (macOS: brew install bash).
       |
       |$functionName() {
  ...
  def handleArgs(args: List[String], executableApp: ExecutableApp): IO[ExecuteError, Unit] = {
    lazy val base: IO[ExecuteError, Unit] =
      args match {
        case "generate" :: "--oxygen-args" :: programArgs =>
          generateScript(programArgs :+ "--").mapError {
            case err: ExecuteError => err
            case err               => ExecuteError.ProgramError(err)
          }
        case "generate" :: "--args" :: programArgs =>
          generateScript(programArgs).mapError {
            case err: ExecuteError => err
            case err               => ExecuteError.ProgramError(err)
          }
        case "generate" :: Nil =>
          generateScript(Nil).mapError {
            case err: ExecuteError => err
            case err               => ExecuteError.ProgramError(err)
          }
        case "complete" :: Nil =>
        ...
        case _ =>
          val msg: String =
            """Invalid special command, supported:
                  |
                  |generate
                  |generate --oxygen-args <program-args*>
                  |generate --args <program-args*>
                  |resolve complete
                  |""".stripMargin
