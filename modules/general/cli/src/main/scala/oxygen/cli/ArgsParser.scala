...
object ArgsParser {
  ...
  final case class Config[A](envVar: String, load: String => Either[String, A], subHelp: SubHelp) extends ArgsParser[A] {
    ...
    override def complete(request: CompletionRequest, value: String): Task[List[String]] = ZIO.succeed(Nil)
  }
  ...
  final case class HelpOnly(combinedHelp: Help) extends ArgsParser[Unit] {
    ...
    override def complete(request: CompletionRequest, value: String): Task[List[String]] =
      ZIO.succeed(if value.isEmpty || value.startsWith("-") then CliHelp.paramNameCompletions(combinedHelp, value) else Nil)
  }
  ...
  final case class Mapped[A, B](parser: ArgsParser[A], f: A => B) extends ArgsParser[B] {
    ...
    override def complete(request: CompletionRequest, value: String): Task[List[String]] = parser.complete(request, value)
  }
  ...
  final def toFinal[A](result: CliParseResult[A, Args]): Either[(CliParseError, Help), A] = result match
    ...
    case CliParseResult.Success(_, remaining)                                  =>
      ...
      (pos, named) match
        ...
        case _ => sys.error(s"ArgsParser.toFinal: unexpected remaining args: positional=$pos, named=$named")
...
object PositionalArgsParser {
  ...
  final case class Single[A](name: String, subHelp: SubHelp, schema: SchemaLike[A], completion: CompletionOptions[A]) extends PositionalArgsParser[A] {
    ...
    override def complete(request: CompletionRequest, value: String): Task[List[String]] =
      completion.completionOptions(value).map(_.toList)
  }
...
object NamedArgsParser {
  ...
  object BuilderLowPriority {
    ...
    trait LowPriority2 extends LowPriority3 {
      ...
      given list: [A] => (pos: PositionalArgsParser.Builder[A]) => Builder[List[A]] = new Builder[List[A]] {
        override def build(name: String, help: SubHelp, shortName: Option[Char]): NamedArgsParser[List[A]] =
          named(name, pos.build(name, help), shortName, help).repeated.withDefault(Nil)
      }

      given nonEmptyList: [A] => (pos: PositionalArgsParser.Builder[A]) => Builder[NonEmptyList[A]] = new Builder[NonEmptyList[A]] {
        override def build(name: String, help: SubHelp, shortName: Option[Char]): NamedArgsParser[NonEmptyList[A]] =
          named(name, pos.build(name, help), shortName, help).repeatedNel
      }
  ...
  final case class Flag(longName: String, absentValue: Boolean, shortName: Option[Char], subHelp: SubHelp) extends NamedArgsParser[Boolean] {
    ...
    override def complete(request: CompletionRequest, value: String): Task[List[String]] =
      ZIO.succeed(if value.isEmpty || value.startsWith("-") then CliHelp.paramNameCompletions(help, value) else Nil)
  }
