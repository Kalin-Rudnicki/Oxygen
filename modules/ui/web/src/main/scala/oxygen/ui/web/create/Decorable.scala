...
abstract class Decorable {
  ...
  trait DecoratorBuilder0 protected[Decorable] () {
    ...
    protected[Decorable] final def make(name: String)(f: Props => Props): Decorator = Decorator._wrapGeneric(GenericDecorator(name)(f))

    protected inline final def focusNodeModifier(name: String)(inline f: Props => NodeModifier): FocusNodeModifier =
      FocusNodeModifier(name, LensUtil.genLens(f))
  ...
  final class FocusNodeModifier @publicInBinary private[Decorable] (name: String, lens: Lens[Props, NodeModifier]) {

    private def genericAdd(label: String, mod: NodeModifier): Decorator = Decorator._wrapGeneric(GenericDecorator(s"$name.nodeModifier.$label")(lens.modify(_ <> mod)))
