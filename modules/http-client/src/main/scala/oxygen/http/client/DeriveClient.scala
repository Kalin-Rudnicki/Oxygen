...
trait DeriveClient[Api] {
  def client(client: HttpClient): Api
}
object DeriveClient {

  def clientLayer[Api: {DeriveClient as der, Tag}]: URLayer[HttpClient, Api] =
  ...
  private[client] def derivedImpl[Api: Type](using Quotes): Expr[DeriveClient[Api]] = {
    ...
    def clientImpl(clientExpr: Expr[HttpClient]): Expr[Api] = {
    ...
    val deriveClientExpr: Expr[DeriveClient[Api]] =
      '{
        new DeriveClient[Api] {
          override def client(client: HttpClient): Api = ${ clientImpl('client) }
        }
      }
