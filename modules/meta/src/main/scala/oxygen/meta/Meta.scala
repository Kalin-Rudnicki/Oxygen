...
final class Meta[Q <: Quotes](val quotes: Q) {
  ...
  sealed trait ParamClause {
    ...
    def params: Either[List[Tree.Statement.Definition.ValDef], List[Tree.Statement.Definition.TypeDef]] = {
      ...
      (tmp2, tmp3) match {
        case (Nil, Nil)      => throw new RuntimeException("Empty ParamClause")
  ...
  object Symbol {
    ...
    sealed abstract class TypeType(final val isEnum: Boolean) {
    ...
    object TypeType {
      ...
      sealed abstract class Sealed(isEnum: Boolean) extends TypeType(isEnum)
      object Sealed {
        def fromSym(sym: Symbol): Option[TypeType.Sealed] = TypeType.fromSym(sym).flatMap(_.toSealed)
      }

      sealed abstract class Case(isEnum: Boolean, final val isObject: Boolean) extends TypeType(isEnum)
      object Case {
        def fromSym(sym: Symbol): Option[TypeType.Case] = TypeType.fromSym(sym).flatMap(_.toCase)
      }

      case object SealedTrait extends TypeType.Sealed(false)
      case object SealedAbstractClass extends TypeType.Sealed(false)
      case object SealedEnum extends TypeType.Sealed(true)

      case object CaseClass extends TypeType.Case(false, false)
      case object CaseObject extends TypeType.Case(false, true)
      case object EnumCaseClass extends TypeType.Case(true, false)
      case object EnumCaseObject extends TypeType.Case(true, true)
      ...
      def fromSym(sym: Symbol): Option[TypeType] = {
        ...
        (
          ...
          case (false, true, false, false, true, false, false) => CaseClass.some
          case (false, true, false, false, false, true, true)  => CaseObject.some
          case (true, true, false, false, true, false, false)  => EnumCaseClass.some
          case (true, true, false, false, false, true, false)  => EnumCaseObject.some
          case (false, false, true, true, _, false, false)     => SealedTrait.some
          case (false, false, true, false, _, false, false)    => SealedAbstractClass.some
          case (true, false, true, false, _, false, false)     => SealedEnum.some
          case _                                               => None
        }
      }
