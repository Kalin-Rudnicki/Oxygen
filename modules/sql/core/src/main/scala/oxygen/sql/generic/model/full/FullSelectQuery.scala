...
object FullSelectQuery {
  ...
  final case class Aggregate(
    ...
    sealed trait RetAggChild {

      final def childToIndentedStr(using Quotes): IndentedString = this match
        case RetAggChild.Self(elem) => IndentedString.inline("", "return-self:", s"< ${elem.expr.show} >")
        case ret: RetAgg            => IndentedString.inline("", ret.toIndentedString)

      final def childToGroupedString(using Quotes): String = this match
        case RetAggChild.Self(elem) => s"< ${elem.expr.show} >"
        case ret: RetAgg            => ret.toGroupedString

    }
    object RetAggChild {

      final case class Self(self: ReturningPart.Elem.Aggregate.ReturnFromSelf) extends RetAggChild

    }

    // FIX-PRE-MERGE (KR) : relocate
    sealed trait RetAgg extends RetAggChild {
    ...
    object RetAgg {
      ...
      final case class AggregateAgg(children: NonEmptyList[RetAggChild]) extends RetAgg {

        override def toGroupedString(using Quotes): String =
          children
            .map(_.childToGroupedString)
            .mkString("[ ", " , ", " ]")
        ...
        override def toIndentedString(using Quotes): IndentedString =
          IndentedString.inline(
            ...
            IndentedString.section(">> " + toGroupedString)(
              children.toList.map(_.childToIndentedStr),
              "",
            ),
          )
          ...
          case value: FullSelectQuery.Aggregate =>
            AggregateAgg(
              value.ret.returningExprsNel.map {
                case elem: ReturningPart.Elem.Aggregate.ReturnFromSelf => RetAggChild.Self(elem)
                case elem: ReturningPart.Elem.Aggregate.NonSelf        => from(elem.select)
              },
            )
        }
    }
