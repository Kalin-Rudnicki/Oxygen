...
final case class InputPart(
    mapQueryRef: QueryReference.InputLike,
) {
  ...
  override def parse(term: Term, refs: RefMap, prevFunction: String)(using ParseContext, Quotes): MapChainParseResult[InputPart] = {
    for {
      (mapAAFC, input) <- AppliedAnonFunctCall.parseTyped[T.InputLike](term, "map function").parseLhsAndFunct {
        case ('{ Q.input.apply[a] }, mapAAFC)            => mapAAFC.funct.parseParam1.map { mapParam => InputPart(QueryReference.InputParam(mapParam)) }
        case ('{ Q.input.const[a](${ expr }) }, mapAAFC) => mapAAFC.funct.parseParam1.map { mapParam => InputPart(QueryReference.ConstInput(mapParam, expr.toTerm, TypeRepr.of[Any])) }
      }
