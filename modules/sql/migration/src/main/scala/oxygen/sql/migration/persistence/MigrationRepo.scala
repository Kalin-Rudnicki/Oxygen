...
trait MigrationRepo {
  ...
  def startMigration(version: Version, now: Instant): IO[ErrorPersistingMigration, Unit]
  def completeMigration(version: Version, now: Instant): IO[ErrorPersistingMigration, Unit]
  def executeStep(version: Version, step: PersistedMigrationFile.Step): IO[StepError, ExecutedMigration.Step]
...
object MigrationRepo {
  ...
  final case class Live(
    ...
    override def startMigration(version: Version, now: Instant): IO[ErrorPersistingMigration, Unit] =
    ...
    override def completeMigration(version: Version, now: Instant): IO[ErrorPersistingMigration, Unit] =
      Live.queries.setCompletedAt.execute(now, version).unit.mapError(ErrorPersistingMigration(version, None, _)).usingDb(db)

    override def executeStep(version: Version, step: PersistedMigrationFile.Step): IO[StepError, ExecutedMigration.Step] = {
      val (effect, sql) = evalStep(version, step)
      for {
        _ <- effect.usingDb(db)
        executedStep = ExecutedMigrationStepRow(version, step.stepNo, step.derived, TypedJsonb(step.step), sql)
    ...
    private def evalStep(version: Version, step: PersistedMigrationFile.Step): (ZIO[Database, ErrorExecutingMigrationStep, Unit], Option[String]) =
      step.step match {
        case diff: MigrationStepColumn.StateDiff =>
          val query = MigrationQueries.diffToQuery(diff.toDomain)
          (query.execute().unit.mapError(ErrorExecutingMigrationStep(version, step, _)), query.ctx.sql.some)
      }
  ...
  object Live {
    ...
    private object queries {
      ...
      val setCompletedAt: QueryI[(Instant, Version)] =
