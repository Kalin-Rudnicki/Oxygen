...
`oxygen-cli-2` annotations live in `oxygen.cli`. `oxygen-executable-2` adds `@command`, `@execute`,
...
Arguments after `--` are preserved for the app (cli-2 merges before/after `--` when parsing).
Unlike v1, v2 does **not** reserve everything before `--` for bootstrap-only flags.